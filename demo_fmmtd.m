...
xno = 51; % number of Fourier modes
yno = 1; % number of Fourier modes
no = xno*yno;
ixy = (ceil(xno/2)-1)*yno+ceil(yno/2);
eps_sub = 1; % substrate permittivity
eps_gr = 1.5^2; % %grating permittivity
...
FE = calc_emntd_lam(xno,yno,0.5,1.0,eps_gr,eps_sup);
...
V_inc = zeros(2*no,2);
V_inc(1*no+ixy,2) = 1; % TM polarized plane wave (0-th harmonic) coming from the superstrate
...
disp("efficiency:");
disp(V_eff(1*no+ixy,1)); % 0th order TM power transmission coefficient
disp(V_eff(1*no+ixy,2)); % 0th order TM power reflection coefficient
